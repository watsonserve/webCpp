 ...
 * args
 */
void* listenEvent(void * args)
{
    struct kevent *eventList;
    register size_t i, nEvent, eventCnt;
    Func way;
    IOInt_t *conf;

    conf = args;
    eventCnt = conf->eventCnt;

    // 可用事件列表
    eventList = (struct kevent *)malloc(sizeof(struct kevent) * eventCnt);
    ...
    {
        // 获取可用事件
        nEvent = kevent(epfd, nullptr, 0, eventList, eventCnt, nullptr);
        for (i = 0; i < nEvent; i++)
        {
